...
public class VirtiusMeetSessionsParser extends AbstractWebParser {
  ...
  private String getPageSource() throws Exception {
    ...
    logger.debug(
        "Meets on Virtius is displayed as an infinite scroll. Scroll to load all content available on the page. Max scroll attempts will be: {}.",
        maxScrollAttempts);
    while (scrollAttempts < maxScrollAttempts) {
      // Get the current scroll height
      Object heightResult = ((JavascriptExecutor) driver)
          .executeScript("return document.documentElement.scrollHeight");
      if (heightResult == null) {
        break;
      }
      long currentHeight = (Long) heightResult;
    ...
    logger.debug("Page loaded. HTML size: {} characters", pageSource != null ? pageSource.length() : 0);
  ...
  private void parseSessions(String pageSource) {
    ...
    if (pageSource == null || pageSource.isEmpty()) {
      logger.error("The source from the web page is null or empty; therefore, the page cannot be parsed.");
      throw new IllegalArgumentException(
          "The source from the web page is null or empty; therefore, the page cannot be parsed.");
    }
    ...
    for (Element heroDiv : heroMessages) {
      ...
      // If the viewSessionLink is null, then look for "View Scores ONLY" link since a
      // few instances
      // reverse the score and session links. Replace a portion of View Scores URL so
      // that it points
      // to the session url:
